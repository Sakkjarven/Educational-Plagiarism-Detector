...
public class PlagiarismDetectionService : IPlagiarismDetectionService
{
    ...
    IProgress<double>? progress = null)
    {
        ...
        // Сравнение документов
        var comparisonResults = CompareDocumentsWithProgress(
            processedDocuments,
            algorithmList,
            progress);
    ...
    private List<ComparisonResult> CompareDocumentsWithProgress(
        List<Document> documents,
        List<SimilarityAlgorithmType> algorithmTypes,
        IProgress<double>? progress)
    {
        ...
        int totalComparisons = (documents.Count * (documents.Count - 1) / 2) * algorithms.Count;
